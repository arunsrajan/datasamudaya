			...
			try {
				...
				CompletableFuture.supplyAsync(() -> {
					try {						
						...
						Stream datastreamrightfirstelem = diskspillmapintermright.isSpilled()
								? (Stream) Utils.getStreamData(diskspillmapintermright)
								: diskspillmapintermright.entrySet().stream();
						Optional optionalfirstelement = datastreamrightfirstelem.findFirst();
						Object[][] nullobjarr;
						if (optionalfirstelement.isPresent()) {
						...
						datastreamrightfirstelem.close();
						streamresult = diskspillmapintermleft.entrySet().stream()
			            .flatMap(entryleft -> {
			                List<Object[]> objectsright = diskspillmapintermright.getOrDefault(entryleft.getKey(), Collections.emptyList());
			                if(CollectionUtils.isNotEmpty(objectsright)) {
			                	List<Object[]> leftvalueobjarr = (List<Object[]>) entryleft.getValue();
								Seq seq1 = Seq.seq(leftvalueobjarr.stream());
								Seq seq2 = Seq.seq(objectsright.stream());
								Seq leftjoinseq = seq1.leftOuterJoin(seq2, leftjoin.getLojp());
								return leftjoinseq.stream();
			                }
						...
						if(CollectionUtils.isNotEmpty(leftjoin.getJoinkeys())) {
							mapdownstream = streamresult.map(new MapFunction<Tuple2<Object[], Object[]>, Object[]>() {
								...
								public Object[] apply(Tuple2<Object[], Object[]> tup2) {
									...
									Object[] rec2 = (Object[]) tup2.v2;
									if (rec2 == null) {
										return new Object[]{SQLUtils.concatenate(((Object[]) tup2.v1()[0]), nullobjarr[0]),
									...
									(Object[] objarr) -> 
									Arrays.asList(SQLUtils.extractMapKeysFromJoinKeys((Object[])objarr[0], leftjoin.getJoinkeys())), ()->new DiskSpillingMap<>(task, "interm"),
