...
public class StreamPipelineUserDefinedObjectsTest extends StreamPipelineBaseTestCommon {
	...
	int sum;
	static Logger log = LogManager.getLogger(StreamPipelineUserDefinedObjectsTest.class);
	...
	public void testMapCollect() throws Throwable {
		...
		List<List<Map>> data = (List<List<Map>>) datapipeline.map(value -> value.split(",")).map(val -> {
