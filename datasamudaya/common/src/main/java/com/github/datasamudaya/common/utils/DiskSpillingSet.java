...
public class DiskSpillingSet<T> extends AbstractSet<T> implements Serializable,AutoCloseable {
	...
	protected void spillToDiskIntermediate(boolean isfstoclose) {
		try {
			...
			if ((Utils.mpBeanLocalToJVM.isUsageThresholdExceeded()
					|| isspilled) && CollectionUtils.isNotEmpty(dataSet)) {
				if (isNull(ostream)) {
					isspilled = true;
	...
	protected void transferDataToDownStreamPipelines() {
		...
		actorselection.tell(new OutputObject(new ShuffleBlock(null,
						Utils.convertObjectToBytes(filepartids.get(filetransferindex)), Utils.convertObjectToBytes(dataSet)), left, right, Dummy.class),
				ActorRef.noSender());
		dataSet.clear();
	}
